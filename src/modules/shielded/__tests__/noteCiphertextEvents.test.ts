...
function inPoolInvoke(...lines: string[]): string[] {
  return [`Program ${POOL} invoke [1]`, ...lines, `Program ${POOL} success`];
}
...
describe('parseNoteCiphertextEvents', () => {
  it('parses 148-byte NoteCiphertext events', () => {
    ...
    const out = parseNoteCiphertextEvents(inPoolInvoke('Program log: x', ncLine(6, ct)));
  ...
  it('ignores non-148-byte program-data lines (LeafInserted 80, Transfer 72)', () => {
    ...
    expect(parseNoteCiphertextEvents(inPoolInvoke(leaf80, transfer72))).toEqual([]);
  });
  it('ignores a line whose len prefix isn\'t 128', () => {
    const buf = Buffer.alloc(8 + 8 + 4 + 128);
    buf.writeUInt32LE(64, 8 + 8); // wrong len
    expect(
      parseNoteCiphertextEvents(inPoolInvoke(`Program data: ${buf.toString('base64')}`)),
    ).toEqual([]);
  });
  ...
  it('REJECTS a correctly-shaped ciphertext forged by another program', () => {
    ...
    const logs = [`Program ${evil} invoke [1]`, ncLine(6, ct), `Program ${evil} success`];
    expect(parseNoteCiphertextEvents(logs)).toEqual([]);
  });
});
