...
export function ConnectPanel() {
  ...
  /*
    Stated on the screen, not only in docs. There is no field here to type a recovery
    phrase into, which is what makes the sentence true rather than a promise — a phishing
    clone cannot claim the real site asks for one. Inside the wallet card in every state
    (the redesign moved it there): it is a promise about this panel, not a page footer.
  */
  const phraseNote = (
    <p className="noc-caption phrase-note">
  ...
  if (publicKey) {
    ...
    return (
      <section aria-labelledby="connect-heading">
        ...
        <div className="noc-card wallet-card is-connected">
          <WalletMultiButton />
          {/* The button already shows GpyR..ay11; the full address is here because a
              truncation is not something a reader can check against anything. */}
          <div className="addr-field">
            <span className="addr noc-mono">{address}</span>
  ...
  if (availability === 'available')
    return (
      <section aria-labelledby="connect-heading">
        ...
        <div className="noc-card wallet-card">
          <p className="noc-body noc-muted">
  ...
  return (
    <section aria-labelledby="connect-heading">
      ...
      <div className="noc-card wallet-card no-wallet">
        <p className="noc-body noc-muted">
        ...
        {/*
          ...
          nowhere. It also keeps the bundle at zero external hosts, which the build gate
          enforces.
        */}
        <p className="noc-caption noc-dim">
        ...
        {/* Set apart: it is for a different reader, the one who already has the app. */}
        <p className="no-wallet-aside noc-body-sm">
